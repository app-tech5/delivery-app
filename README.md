# Good Food Pro — Driver App

Expo / React Native driver client for the Good Food Pro food-delivery suite.
Drivers accept deliveries, navigate, update status, and manage earnings against the **Good Food Pro REST API** (Express + MongoDB).
...
**Expo Go is not supported.** This app uses a custom native stack (`expo-dev-client`, MapLibre, background location, etc.). Install a **development build** once, then use Metro for day-to-day JS changes.
...
Then open the **Good Food Pro Driver** app already on the device/emulator (or press `a` / `i` if Expo can resolve your installed development build).
...
**v1.0.0** — Good Food Pro Driver App
